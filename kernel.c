...
struct process *current_process, *idle_process, *proc_a, *proc_b;
...
struct sbret sbi_call(
    ...
    return (struct sbret){.error = a0, .value = a1 };
}
...
long getchar(void) {
    struct sbret ret = sbi_call(0, 0, 0,0 ,0,0,0,2);
    return ret.error;
}
...
struct process *create_process(const void *image, size_t image_size) {
    ...
    *--sp = (uint32_t) user_entry;          // ra
    ...
    printf("create process: pid=%d idx=%d\n", proc->pid, i);
    return proc;
}
...
void yield(void) {
    ...
    for(int  i = 0; i < PROCS_MAX; i++) {
        int idx  = (current_process->pid + i) % PROCS_MAX;
...
void handle_syscall(struct trap_frame *f) {
    switch (f->a3) {
        ...
        case SYS_EXIT: {
            ...
            current_process->state = PROCS_EXITED;
            yield();
            PANIC("unreachable");
            break;
        }
        ...
        default: {
            PANIC("unimplemented syscall a3=%x\n", f->a3);
        }
    }
}
...
void kernel_main(void) {
    ...
    idle_process = create_process(NULL, 0);
    idle_process->pid = 0;
